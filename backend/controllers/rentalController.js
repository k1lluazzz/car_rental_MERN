...
const updateRentalStatus = async (req, res) => {
  try {
    ...
    if (
      ["completed", "returned"].includes(req.body.status) &&
      !["completed", "returned"].includes(oldStatus)
    ) {
...
const returnCar = async (req, res) => {
  try {
    ...
    if (rental.status !== "completed") {
      return res
        .status(400)
        .json({ message: "Đơn thuê không ở trạng thái có thể trả xe" });
    }
...
const addReview = async (req, res) => {
  try {
    ...
    if (!["completed", "returned"].includes(rental.status)) {
      return res
        .status(400)
        .json({ message: "Chỉ có thể đánh giá sau khi hoàn thành thuê xe" });
    }
    ...
    const totalRating = car.reviews.reduce(
