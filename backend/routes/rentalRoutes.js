...
router.post("/:id/change-car", authenticateToken, async (req, res) => {
  try {
    ...
    // Verify ownership
    if (
      oldRental.userId.toString() !== req.user.id &&
      req.user.role !== "admin"
    ) {
    ...
    const isAvailable = await Rental.isCarAvailable(
      carId,
      newStartDate,
      newEndDate
    );
    if (!isAvailable) {
      return res
        .status(400)
        .json({ message: "Selected car is not available for these dates" });
    }
...
router.post("/book", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const populatedRental = await Rental.findById(newRental._id).populate(
      "car"
    );
...
router.get("/user/:userId", authenticateToken, async (req, res) => {
  try {
    ...
    const rentals = await Rental.find({
      ...
      $or: [{ status: "pending" }, { status: "unpaid" }],
    }).populate("car");
    res.json(rentals);
  } catch (err) {
    res.status(500).json({ message: err.message });
