...
// Get rental schedule for a specific car
router.get("/:id/schedule", authenticateToken, async (req, res) => {
  try {
    const rentals = await Rental.find({
      ...
      .select("startDate endDate durationInDays status")
      .sort({ startDate: 1 });

    res.json(rentals);
  } catch (error) {
    console.error("Error fetching car schedule:", error);
    res.status(500).json({ message: "Error fetching car schedule" });
  }
});
